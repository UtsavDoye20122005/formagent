...
import { mergeFields } from "./schema.js";
...
// Deleting answers. The database's own rules already refuse a delete on a form
// somebody does not own, but we check first anyway so the caller gets an honest
// "not found" rather than a silent no-op that looks like success.
export async function deleteResponses(formId, ids) {
  const { supabase } = await authed();
  ...
  const { error, count } = await supabase
    ...
    .delete({ count: "exact" })
    .eq("form_id", formId)
    .in("id", wanted.slice(0, 500));

  if (error) throw new Error(error.message);
  return { deleted: count ?? wanted.length };
}

// Emptying a form completely. Separate from the above so that "delete these
// three" can never turn into "delete everything" through an empty array.
...
export async function addPublicResponse(formId, answers, ipHash = null) {
  ...
  if (error) {
    ...
    if (raw.includes("already_sent"))
      throw new Error("We already have this response — it looks like it was sent twice.");
    if (raw.includes("file_not_uploaded"))
      throw new Error("Your file did not finish uploading. Attach it again and send.");
    throw new Error(raw);
  }
}

// Editing the questions of a form that is already out in the world. The rules
// themselves live in lib/schema.js so they can be tested on their own and so the
// browser can warn before saving instead of after.
export async function updateFormFields(id, incoming) {
  const { supabase, user } = await authed();

  const form = await getOwnForm(id);
  if (!form) return null;

  const { fields, error: refusal } = mergeFields(
    form.fields,
    incoming,
