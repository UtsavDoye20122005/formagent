...
const FORM_COLS =
  ...
  "is_open, closes_at, max_responses, response_count, accent, logo_url, created_at";
...
export async function updateForm(id, patch) {
  ...
  if (patch.maxResponses !== undefined) {
    const raw = String(patch.maxResponses ?? "").trim();
    if (!raw) {
      row.max_responses = null;
    } else {
      const n = Number(raw);
      if (!Number.isFinite(n) || n < 1 || n > 100000) {
        throw new Error("The response limit has to be a number between 1 and 100000.");
      }
      row.max_responses = Math.floor(n);
    }
  }
...
export async function getPublicForm(id) {
  ...
  const { data, error } = await client
    .from("forms")
    .select(
      "id, title, description, submit_label, thank_you, fields, is_open, closes_at, max_responses, response_count, accent, logo_url"
    )
    .eq("id", id)
    .maybeSingle();
  ...
  if (data.is_open === false) {
    ...
    (data.response_count ?? 0) >= data.max_responses
  ) {
    closedReason = "full";
  }
...
export async function addPublicResponse(formId, answers) {
  ...
  const { error } = await client.from("responses").insert({ form_id: formId, answers });
  ...
  if (error) {
    ...
    if (raw.includes("form_full")) throw new Error("This form has reached its limit of responses.");
    throw new Error(raw);
  }
}
